...
const countArrows = (lastPoint: number, points: number[][]): number =>
    points.length === 0
        ? 1
...
const findMinArrowShots_with_ptr = (points: number[][]): number => {
    const sorted = points.sort((left, right) => left[0] === right[0] ? left[1] - right[1] : left[0] - right[0]);
    return countArrows_with_ptr(sorted[0][1], sorted, 1);
};

const countArrows_with_ptr = (lastPoint: number, points: number[][], idx: number): number => {
